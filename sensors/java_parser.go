...
func walkJavaNodes(n *sitter.Node, content []byte, violations *[]Violation) {
	...
	if t == "method_declaration" || t == "constructor_declaration" {
		...
		*violations = append(*violations, Violation{RuleName: "ArgumentCount", Value: countJavaParams(n), StartLine: startLine, EndLine: endLine})
		*violations = append(*violations, Violation{RuleName: "Complexity", Value: countJavaComplexity(n, content), StartLine: startLine, EndLine: endLine})
	}
