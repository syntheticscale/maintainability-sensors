...
- **During Legacy Audits (Audit Mode):** If you are auditing legacy code and believe that refactoring it to pass the strict baseline would constitute "metric overfitting" (i.e., breaking a highly coherent, battle-tested structure), you **MUST stop and ask the human user for explicit permission** before adjusting any thresholds. Use the `ask_user` tool or simply pause and present your architectural reasoning. If granted permission, you should strongly prefer **"Ratchet B" (File-Specific Configuration Overrides)** over modifying global thresholds. For example, add a per-path override in `.golangci.yml` or `eslint.config.js` locking the specific legacy file to its current complexity score. This creates a ratchet that prevents the file from degrading further, while keeping the global repository threshold strictly secure. Do NOT sweep technical debt under the rug without human approval.
