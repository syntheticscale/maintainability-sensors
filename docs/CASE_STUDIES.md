...
2.  **We capture the "Why" through exceptions and ratchets:** If an engineering team has a genuine performance reason for writing a complex loop (e.g., in a core web server routing trie), our system doesn't block them. They can use localized inline exceptions (`//nolint:gocognit`). However, for massive legacy audits, we employ **"Ratchet B" (File-Specific Configuration Overrides)**. Instead of relaxing global repository thresholds, teams add a per-path override in their configuration file locking the legacy file to its current complexity score. This creates a hard ratchet that prevents the file from degrading further, while explicitly documenting the architectural trade-off.
