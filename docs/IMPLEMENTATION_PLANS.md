...
### 2. Universal AST via Tree-sitter
*   **Context:** Relying on external linters (ESLint, pylint) requires the agent to have language-specific environments installed, and introduces execution overhead.
*   **Implementation:** Integrate `go-tree-sitter` to natively parse TypeScript, Python, etc., entirely within the Go binary.
*   **Execution Plan:**
    *   Calculate Cyclomatic Complexity, Function Length, and Parameter limits deterministically in memory using Tree-sitter grammars.
    *   Gradually deprecate the orchestration of external tools for these core metrics to achieve true sub-millisecond, polyglot execution with zero dependencies.
